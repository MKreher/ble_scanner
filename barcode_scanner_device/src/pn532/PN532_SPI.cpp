...
{
  nrf_gpio_cfg_output(PN532_SPI_SS_PIN);
  nrf_gpio_cfg_input(PN532_IRQ_PIN, NRF_GPIO_PIN_NOPULL);
       
  // regarding spi config see page 25 in pn532 datasheet
  nrf_drv_spi_config_t spi_config = 
    {
      spi_config.sck_pin = PN532_SPI_SCK_PIN,
      spi_config.mosi_pin = PN532_SPI_MOSI_PIN,
      spi_config.miso_pin = PN532_SPI_MISO_PIN,
      spi_config.ss_pin = PN532_SPI_SS_PIN,
...
{    
    nrf_gpio_pin_clear(PN532_SPI_SS_PIN);
    nrf_delay_ms(2);
    nrf_gpio_pin_set(PN532_SPI_SS_PIN);
}
