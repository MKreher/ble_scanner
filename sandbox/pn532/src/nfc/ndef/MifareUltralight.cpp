...
NfcTag MifareUltralight::read(uint8_t * uid, uint16_t uidLength)
{
    if (isUnformatted())
    {
#ifdef NDEF_USE_SERIAL
        NRF_LOG_INFO("WARNING: Tag is not formatted.");
#endif
...
void MifareUltralight::readCapabilityContainer()
{
    ...
    uint16_t success = nfc->mifareultralight_ReadPage (3, data);
    if (success)
    {
        // See AN1303 - different rules for Mifare Family byte2 = (additional data + 48)/8
        tagCapacity = data[2] * 8;
        #ifdef MIFARE_ULTRALIGHT_DEBUG
       NRF_LOG_INFO("Tag capacity %d bytes", tagCapacity);
        #endif
...
bool MifareUltralight::write(NdefMessage& m, uint8_t * uid, uint16_t uidLength)
{
    if (isUnformatted())
    {
#ifdef NDEF_USE_SERIAL
	...
	#ifdef MIFARE_ULTRALIGHT_DEBUG
    	NRF_LOG_INFO("Encoded Message length exceeded tag Capacity %d", tagCapacity);
    	#endif
    	return false;
    }
    ...
    #ifdef MIFARE_ULTRALIGHT_DEBUG
    NRF_LOG_INFO("messageLength %d", messageLength);
    NRF_LOG_INFO("Tag Capacity %d", tagCapacity);
...
bool MifareUltralight::clean()
{
    ...
    for (uint16_t i = ULTRALIGHT_DATA_START_PAGE; i < pages; i++) {
        #ifdef MIFARE_ULTRALIGHT_DEBUG
        NRF_LOG_INFO("Wrote page %i - ", i)
