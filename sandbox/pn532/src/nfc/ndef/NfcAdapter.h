...
class NfcAdapter {
    public:
        ...
        bool tagPresent(uint32_t timeout=0); // tagAvailable
        NfcTag read();
    ...
    private:
        ...
        uint16_t uidLength; // Length of the UID (4 or 7 bytes depending on ISO14443A card type)
        uint16_t guessTagType();
};
