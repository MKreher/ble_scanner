...
NfcTag MifareClassic::read(uint8_t *uid, uint16_t uidLength)
{
    uint8_t key[6] = { 0xD3, 0xF7, 0xD3, 0xF7, 0xD3, 0xF7 };
    uint16_t currentBlock = 4;
    uint16_t messageStartIndex = 0;
    uint16_t messageLength = 0;
    uint8_t data[BLOCK_SIZE];

    // read first block to get message length
    uint16_t success = _nfcShield->mifareclassic_AuthenticateBlock(uid, uidLength, currentBlock, 0, key);
    if (success)
    {
    ...
    uint16_t index = 0;
    uint16_t bufferSize = getBufferSize(messageLength);
...
uint16_t MifareClassic::getBufferSize(uint16_t messageLength)
{

    uint16_t bufferSize = messageLength;

    // TLV header is 2 or 4 bytes, TLV terminator is 1 uint8_t.
...
uint16_t MifareClassic::getNdefStartIndex(uint8_t *data)
{
    ...
    for (uint16_t i = 0; i < BLOCK_SIZE; i++)
    {
        ...
        {
#ifdef NDEF_USE_SERIAL
            NRF_LOG_INFO("Unknown TLV %x", data[i]);
#endif
            return -2;
        }
    }
...
bool MifareClassic::decodeTlv(uint8_t *data, uint16_t &messageLength, uint16_t &messageStartIndex)
{
    uint16_t i = getNdefStartIndex(data);
...
bool MifareClassic::formatNDEF(uint8_t * uid, uint16_t uidLength)
{
...
#define NR_LONGSECTOR           (8)     // Number of uint32_t sectors on Mifare 4K
#define NR_BLOCK_OF_SHORTSECTOR (4)     // Number of blocks in a short sector
#define NR_BLOCK_OF_LONGSECTOR  (16)    // Number of blocks in a uint32_t sector
...
bool MifareClassic::formatMifare(uint8_t * uid, uint16_t uidLength)
{
...
bool MifareClassic::write(NdefMessage& m, uint8_t * uid, uint16_t uidLength)
{
    ...
    while (index < sizeof(buffer))
    {
        ...
        uint16_t write_success = _nfcShield->mifareclassic_WriteDataBlock (currentBlock, &buffer[index]);
        if (write_success)
        {
            #ifdef MIFARE_CLASSIC_DEBUG
